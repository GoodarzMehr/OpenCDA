...
CARLA_EGG_FILE=${CARLA_HOME}/PythonAPI/carla/dist/carla-"${CARLA_VERSION}"-py3.10-linux-x86_64.egg
...
echo "unzip egg file"
unzip "${CACHE}"/carla-"${CARLA_VERSION}"-py3.10-linux-x86_64.egg -d "${CACHE}"/carla-"${CARLA_VERSION}"-py3.10-linux-x86_64

echo "copy setup file to egg folder"
SETUP_PY=${PWD}/scripts/setup.py
cp "$SETUP_PY"  "${CACHE}"/carla-"${CARLA_VERSION}"-py3.10-linux-x86_64/

echo "Successful! Run 'pip install -e ${CACHE}/carla-${CARLA_VERSION}-py3.7-linux-x86_64' to install carla into your python package "
conda activate opencdaP310
pip install -e ${CACHE}/carla-"${CARLA_VERSION}"-py3.10-linux-x86_64
